...
export default function DeleteAccount({ onDelete, className, children }) {
  ...
  return (
    <>
      ...
      {open && <DeleteAccountDialog onDelete={onDelete} onClose={() => setOpen(false)} />}
    </>
  );
}

/** The confirmation dialog on its own, for callers with their own trigger (the profile menu). Rendered into <body> so the header
 * band (overflow-hidden, isolate) cannot clip it or keep it under the cards. */
export function DeleteAccountDialog({ onDelete, onClose }) {
  const { t } = useTranslation();
