...
1. **Clinical validation.** `validation/REPORT.md` is a technical validation on one public dataset (95.1% sensitivity, 89.2% specificity on held-out images; it also documents the model's misses, including 1 of 44 proliferative cases). It is not clinical validation: the model has not been tested on your cameras or patients, and it must not be the sole basis for any decision.
