function [referableProb, heatmapEmpty] = gradCamToFile(imgPath, outPath, analysedPath)
...
%   analysedPath    optional: also save the prepared (cropped, 224x224) image the network analysed, for the PDF report
%   heatmapEmpty    true when no region raised the referral score (the map is all zeros and the image is saved without colour)
    ...
    if nargin >= 3 && ~isempty(analysedPath)
        imwrite(result.preprocessedImage, analysedPath);
    end
end
